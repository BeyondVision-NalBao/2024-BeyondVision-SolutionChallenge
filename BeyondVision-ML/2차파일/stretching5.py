...
def check_backbone(keypoint):
    backbone=abs(keypoint[19][1]-((keypoint[15][1]+keypoint[16][1])/2))
    value = 15

    if  backbone <= backbone_LIMIT + value:
        return True
    # elif hip_knee > d_LIMIT + value:
    #     
    #     message = "조금 일어나세요.")
    #     return False
    elif backbone_LIMIT + value < backbone:
        
        message = "골반이 정중앙에 오도록 해주세요"
        return False



def postureCorrection(keypoint):
    if rightArm_position(keypoint) and leftArm_position(keypoint) and check_backbone(keypoint):
